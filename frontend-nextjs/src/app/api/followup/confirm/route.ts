 ...
 * 
 * GET /api/followup/confirm?id=appointmentId
 * 
 * Confirms a follow-up appointment (from email link or direct).
 * Updates status to confirmed with timestamp.
 */
...
export async function GET(request: NextRequest) {
  ...
  if (!appointmentId && !token) {
    return renderPage('error', 'Invalid confirmation link.');
  }
  ...
  try {
    ...
    if (token) {
      ...
      if (!tokenRecord) {
        return renderPage('error', 'Invalid or expired confirmation link.');
      }

      if (tokenRecord.used) {
        return renderPage('info', 'This appointment has already been confirmed.');
      }

      if (new Date() > tokenRecord.expires_at) {
        return renderPage('error', 'This confirmation link has expired. Please contact the clinic.');
      }
    ...
    if (appointment.confirmation_status === 'confirmed') {
      return renderPage('info', 'Your appointment is already confirmed. Thank you!');
    }

    if (appointment.status === 'cancelled') {
      return renderPage('error', 'This appointment has been cancelled.');
    }
    ...
    const timeStr = appointment.appointment_time
      ? new Date(appointment.appointment_time).toLocaleTimeString('en-IN', { hour: 'numeric', minute: '2-digit', hour12: true })
      : 'To be confirmed';

    return renderPage(
      'success',
      `Your follow-up appointment on <strong>${dateStr}</strong> at <strong>${timeStr}</strong> has been confirmed.`,
      appointment.patient.full_name
    );
  } catch (error) {
    console.error('[Followup Confirm] Error:', error);
...
function renderPage(
  ...
  const colors = {
    success: { bg: '#dcfce7', icon: '✅', heading: 'Appointment Confirmed!' },
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .patient-name { font-weight: 600; color: #004A99; }
